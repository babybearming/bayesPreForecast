...
#    T,c1,c0,afang,dfang=uzpara(obs1,fore1)
    ...
    return y0,gypmf,gycdf,Faiydata
...
stanums=255;varnums=51;foredaynums=1;

startyr=2017  #模型建立所用资料，此处为2年
endyr=2018

obsdata=getobsdata(startyr,endyr)

rs=np.load('./results/rs.npy')   #获取前期计算得到的集合预报各成员信息权重
rsfaipmf=[i for i in range(stanums)]
rspypmf=[i for i in range(stanums)]

for sta in range(stanums):
#    sta=247  #143石家庄站,254天津  54525  247  
    print(sta)
    g1=g(1981,2010,sta)
    ...
    for foreday in range(foredaynums):
        ...
        faipmf=[i for i in range(varnums)]
        rsfaipmf1=0
        pypmf_=[i for i in range(varnums)]
        rspypmf1=0
    ...
    #        x=100
            ...
            if x==0:
                ...
                y,gypmf,gycdf,Faiydata=calFaiy(obs,fore,x) #0.077
                
                paivalue=Pai(g1,obs,fore,x,datenum)  #0.022  
                pyvalue=Pyconditionx(paivalue,Faiydata)
                
                faipmf[varnum]=faimidu(Faiydata)
                pypmf_[varnum]=pypmf(pyvalue)
                
        rs_sta_foreday=rs[sta,foreday,:]
        rs_sta_sum=np.sum(rs_sta_foreday)
        rs_sta_foreday_gui1=rs_sta_foreday/rs_sta_sum
        for varnum1 in range(varnums):
            rsfaipmf1=rsfaipmf1+faipmf[varnum1]*rs_sta_foreday_gui1[varnum1]
            rspypmf1=rspypmf1+pypmf_[varnum1]*rs_sta_foreday_gui1[varnum1]
        rsfaipmf1sum=np.sum(rsfaipmf1)
        rspypmf1sum=np.sum(rspypmf1)
        if rsfaipmf1sum==0:
            rsfaipmf[sta]=0
        else:
            rsfaipmf[sta]=rsfaipmf1/rsfaipmf1sum
        if rspypmf1sum==0:
            rspypmf[sta]=0 
        else:
            rspypmf[sta]=rspypmf1/np.sum(rspypmf1)
        
        results_path='./results/'+nowdate.strftime('%Y%m%d')+'/'
        if os.path.exists(results_path)==False:
            os.makedirs(results_path)
        np.save(results_path+'rsfaipmf_'+str(foreday+1)+'day.npy',rsfaipmf)
        np.save(results_path+'rspypmf_'+str(foreday+1)+'day.npy',rspypmf)
